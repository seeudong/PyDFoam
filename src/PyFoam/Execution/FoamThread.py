...
class FoamThread(Thread):
    ...
    def read(self):
        ...
        line_read, interrupted = self.reader.read_and_interrupt(self.output)
        self.setState(line_read, interrupted=interrupted, pass_int=not self.keyboardInterupted)
    ...
    def interrupt(self, pass_int=False):
        """A keyboard-interrupt is reported"""
#        print "Interrupt"
        self.reader.wasInterupted = True
        self.setState(False, pass_int=pass_int)

    def setState(self, state, pass_int=False, interrupted=True):
        ...
        if not self.hasSomethingToSay and self.timeStart and self.reader.wasInterupted:
            ...
            if self.threadPid>0 and interrupted:
                if pass_int:
                    msg = "Passing SIGINT to %d" % self.threadPid
                    sig = signal.SIGINT
                else:
