    ...
    def start(self):
        ...
        while self.run.check():
            try:
                ...
                e = sys.exc_info()[1] # compatible with 2.x and 3.x
                if ctrl_c_pressed:
                    msg = "- Second Keyboard-Interrupt. Killing process"
                else:
                    msg = "- First Keyboard-Interrupt. Allowing process to finish gracefully. Next Keyboard interrupt kills unconditionally"
                foamLogger().warning("Keyboard Interrupt " + msg)
                self.run.interrupt(pass_int=ctrl_c_pressed)
