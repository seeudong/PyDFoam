...
class CommonClearCase(object):
    ...
    def addOptions(self):
        ...
        self.generalOpts.add_option("--keep-dynamic-code",
                                    action="store_true",
                                    dest="keepDynamicCode",
                                    default=False,
                                    help="Keep the directory 'dynamicCode' where compiled stuff is written")
    ...
    def clearCase(self,sol,runParallel=False):
        ...
        if not self.opts.keepDynamicCode:
            self.opts.additionalClear.append("dynamicCode")
