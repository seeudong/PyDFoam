...
def doImports():
    ...
    try:
        ...
        try:
            ...
            from PyQt5 import QtGui as QtGui_real

        except ImportError:
            from PyQt4 import QtGui,QtCore
            from PyQt4.QtGui import QMainWindow
            from PyQt4 import QtGui as QtGui_real
...
class UtilityThread(QtCore.QThread):
    ...
    # Might not work for Qt before 5
...
class DisplayBlockMeshDialog(QMainWindow):
    def __init__(self,
                 fName,
                 valuesFile=None,
                 opts=None):
        ...
        try:
            ...
            self.rereadAction.triggered.connect(self.reread)
        ...
        try:
            ...
            self.blockMeshAction.triggered.connect(self.blockMesh)
        ...
        try:
            ...
            self.checkMeshAction.triggered.connect(self.checkMesh)
        ...
        try:
            ...
            self.saveAction.triggered.connect(self.saveBlockMesh)
        ...
        self.editor.setFont(QtGui_real.QFont("Courier"))
        ...
        if self.vName:
            ...
            self.vEditor.setFont(QtGui_real.QFont("Courier"))
        ...
        self.utilityOutput.setFont(QtGui_real.QFont("Courier"))
        ...
        try:
            self.connect(
                ...
                self.numberScaleChanged)
        except AttributeError:
            nrScale.valueChanged.connect(self.numberScaleChanged)
        ...
        try:
            self.connect(
                ptScale,
                QtCore.SIGNAL("valueChanged(double)"),
                self.pointScaleChanged)
        except AttributeError:
            ptScale.valueChanged.connect(self.pointScaleChanged)
        ...
        try:
            self.connect(
                axisLScale,
                QtCore.SIGNAL("valueChanged(double)"),
                self.axisLabelScaleChanged)
        except AttributeError:
            axisLScale.valueChanged.connect(self.axisLabelScaleChanged)
        ...
        try:
            self.connect(
                axisTScale,
                QtCore.SIGNAL("valueChanged(double)"),
                self.axisTubeScaleChanged)
        except AttributeError:
            axisTScale.valueChanged.connect(self.axisTubeScaleChanged)
        ...
        try:
            ...
            self.scroll.valueChanged.connect(self.colorBlock)
        ...
        try:
            ...
            self.scroll2.valueChanged.connect(self.colorPatch)
        ...
        try:
            ...
            self.rereadButton.clicked.connect(self.reread)
    ...
    def setupBlockingGui(self):
        ...
        try:
            self.connect(
                self.defineBlockButton,
                QtCore.SIGNAL("clicked()"),
                self.defineBlock)
        except AttributeError:
            self.defineBlockButton.clicked.connect(self.defineBlock)
        ...
        try:
            ...
            self.insertBlockButton.clicked.connect(self.insertBlock)
        ...
        self.hexeditor.setFont(QtGui_real.QFont("Courier"))
        ...
        try:
            self.connect(
                self.definePatchButton,
                QtCore.SIGNAL("clicked()"),
                self.definePatch)
        except AttributeError:
            self.definePatchButton.clicked.connect(self.definePatch)

        self.insertPatchButton=QtGui.QPushButton("Insert Patch")
        displayLayout.addWidget(self.insertPatchButton,5,0)
        try:
            self.connect(
                self.insertPatchButton,
                QtCore.SIGNAL("clicked()"),
                self.insertPatch)
        except AttributeError:
            self.insertPatchButton.clicked.connect(self.insertPatch)
        ...
        try:
            self.connect(
                self.reverseNormalButton,
                QtCore.SIGNAL("clicked()"),
                self.reverseNormal)
        except AttributeError:
            self.reverseNormalButton.clicked.connect(self.reverseNormal)
    ...
    def blockMesh(self):
        if self.vName:
            fName = path.splitext(self.fName)[0]
        else:
            fName = self.fName
        self.executeUtility("blockMesh", args=["-dict", fName])

    def checkMesh(self):
        self.executeUtility("checkMesh")

    def executeUtility(self,util, args=[]):
        if self.worker!=None:
        ...
        try:
            ...
            self.worker.finished.connect(self.executionEnded)
        try:
            self.connect(
                self.worker,
                QtCore.SIGNAL("newLine(QString)"),
                self.utilityOutputAppend)
        except AttributeError:
            self.worker.newLine.connect(self.utilityOutputAppend)
        self.worker.start()
    ...
    def readFile(self,resetText=True):
        ...
        if self.vName:
            ...
            template=TemplateFile(self.fName,
                                  engine=self.opts.templateDefaultEngine,
                                  fallback_engine=self.opts.fallbackDefaultEngine,
                                  peek_lines=self.opts.peekLinesAuto,
    ...
    def pointScaleChanged(self,scale):
        ...
        for tActor, coord in zip(self.tActors, self.vertices):
            old = tActor.GetPosition()
            tActor.AddPosition(coord[0]+self.vRadius-old[0],
                               coord[1]+self.vRadius-old[1],
                               coord[2]+self.vRadius-old[2])
        self.renWin.Render()
    ...
    def makeArc(self,data):
        if len(data) == 3:
            ...
            warning("Can't render an arc with this specification", data)
